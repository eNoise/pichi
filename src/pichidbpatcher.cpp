...
void PichiDbPather::initDbStruct(void )
{
	...
	sql->exec("INSERT INTO settings (`name`, `value`, `description`) VALUES ('onstart_client_detection','0','" + sql->escapeString((*lang)["db_configdesc_onstart_client_detection"]) + "');"); // определение клиента при старте
}
